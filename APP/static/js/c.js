...
function Process(json_value){
  //SetTab();
  window.TAB = MultiplyString(parseInt('4'), window.SINGLE_TAB);
  ...
  try{
   ...
   var html_vvalue = "<PRE class='CodeContainer'>"+html+"</PRE>";
  }catch(e){
    alert("JSON数据格式不正确:\n"+e.message);
    var html_vvalue  = "";
      //$id("RawJson").innerHTML
  }
    return html_vvalue
}
...
function SetTab(){
    //$id("TabSize") select.options[select.selectedIndex].value
  //var select = 4;
